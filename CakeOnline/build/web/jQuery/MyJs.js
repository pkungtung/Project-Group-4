...
jQuery(window).scroll(function () { //Bắt sự kiện cuộn trang
    if (jQuery(window).scrollTop() > 500) { //Kiểm tra vị trí cuộn trang (Cách top 700px)
        jQuery('#autoTop').animate({'width': 'show'}, 500);  //Nếu cách top 700px thì gán thuộc tính right:10px cho thẻ div để hiển thị
    } else {
        jQuery('#autoTop').animate({'width': 'hide'}, 500); //Nếu cách top dưới 700px thì gán thuộc tính right:-80px để ẩn thẻ div
    }
});
